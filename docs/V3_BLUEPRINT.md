...
- **Calendar / free-busy awareness (two ideas, keep them separate).** Belongs to **Persona**, not The Wire — a free-busy signal is cross-app (the meal-planner and reminders want it too), so it lands as a Persona trait/coarse-state source and every client consumes it.
  - *(easy, high-value) Free-busy → the interrupt gate + scheduled editions.* The trust ladder already reasons over coarse states (`meeting`, `open`); a calendar is simply the richest source of them. This is an **extension of the existing coarse-state model, not new architecture** — the device derives "busy until 15:00" and reports only that. Upgrades: never interrupt during a meeting; hold the p3 for the next gap; deliver the morning digest in the commute window and the wind-down when free. Privacy-first path is **iOS EventKit** (raw calendar never leaves the phone, per §8); a **Google Calendar OAuth** path is the non-iOS fallback but sends more to the server and adds token-refresh surface, so it's the weaker option.
  - *(novel, the real bet) Event-relevant local news + slot placement.* Match the place+time-tagged locality triggers the signals worker ALREADY produces (weather-at-home, planning-near-home, energy windows, future travel) to calendar events by (place, time): "frost the morning of your 8am meeting — leave early", "rail disruption on your Cardiff-trip Saturday", "a gig near your evening plans". This turns the brief's "interview at 12, leave by 11:20, M4 is slow" into reality. The weaker cousin — filling free gaps with longform reads matched to the slot length ("20 min free at 4pm") — is **optional and must be restrained**: the calm/doomscroll-averse persona would hate aggressive slot-filling, so event-relevance and busy-respect are the value, not gap-stuffing.
  - Gated on either the iOS app (EventKit) or a deliberate Google Calendar integration; a Phase-2/3+ item, after pitch-level rendering and the a11y sweep.
